...
# Lab cell names
lab.cell.names <- read.csv(paste0(work_dir,"cell_annotation_all.csv") , na.strings = "") # this file is "cell_annotation_all.csv" from pachy annotation 
...
cell<-read.delim("https://discover.nci.nih.gov/cellminer/samples/NCI60_CELL_LINE_METADATA.txt" , skip = 7, check.names =F)[c(1:60),]
...
rna <- read_excel(unzip(rna.file, "output/RNA__Affy_HG_U133_Plus_2.0_RMA.xls") , skip =10)
...
mirna <- read_excel(unzip(mirna.file, "output/RNA__Agilent_Human_microRNA_(V2)_GeneSpringGX.xls") ,skip = 10)
...
rnaseq.comp <- read_excel(unzip(rnaseq.comp.file, "output/RNA__RNA_seq_composite_expression.xls") ,skip = 10)
...
rnaseq.iso <- read_excel(unzip(rnaseq.iso.file, "output/RNA__RNA_seq_isoforms.xls") ,skip = 10)
...
########################################### Cell object ##########################################################
dose.resp<-fread(paste0(work_dir,"DOSERESP.csv"))
...
########################################### Drug object ##########################################################
drug_with_ids <- read.csv(paste0(work_dir,"drug_with_ids.csv"), stringsAsFactors = F , na.strings = "") # this file is "drug_with_ids.csv" from pachy-annotations
...
##################### Sensitivity - all #####################
raw.sensitivity <-readRDS(paste0(work_dir,"raw.sensitivity_v3.rds"))
raw.sensitivity[,,"Dose"] <- raw.sensitivity[,,"Dose"]* 1000000 # convert to micromolar

profile.sensitivity <-readRDS(paste0(work_dir,"profile.sensitivity_v3.rds"))
...
saveRDS(NCI60_PSet, paste0(work_dir,"NCI60.rds"))
